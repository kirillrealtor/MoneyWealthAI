...
function Row({ n, onRead }: { n: Notification; onRead: () => void }) {
  ...
  return (
    <Panel className={cn("group flex items-start gap-3 py-4", !n.is_read && "ring-1 ring-brand/20")}>
      <span className="grid size-9 shrink-0 place-items-center rounded-xl bg-hover ring-1 ring-line">
      ...
      {!n.is_read && (
        <button
          ...
          className="grid size-8 shrink-0 place-items-center rounded-lg text-fg-subtle opacity-0 transition-opacity hover:bg-hover hover:text-fg group-hover:opacity-100"
        >
