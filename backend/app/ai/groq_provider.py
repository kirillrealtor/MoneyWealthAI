"""Groq provider — OpenAI-compatible tool-calling loop behind the same
AdvisorProvider interface as Claude.

Groq serves open models (Llama, etc.) on a free/cheap tier. Same MCP tool
JSON, converted to OpenAI's function-tool shape. IMPORTANT: open models are
weaker at compliance framing and tool grounding, so the output validator and
evals matter MORE here — this is a dev / fallback path, not a drop-in equal of
Claude for a financial advisor.
"""
...
class GroqProvider:
    ...
    async def run(
        ...
        *,
        ...
        logger.warning("groq tool rounds exhausted", service="ai-advisor", tool_calls=tool_calls)
        return AdvisorResult(
